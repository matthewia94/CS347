...
{
  bool less = true;
  if((rhs.computeHeur() + rhs.m_move.m_depth) > (computeHeur()+ m_move.m_depth))
    less = false;
  return less;
}
...
void Puzzle::solveAStarGS()
{
      ...
      for(int j = 0; j < 4; j++)
      {
           ...
           temp.m_wrigglers[i].m_headY))
        {
          if(!temp.inVisited(visited))
          {
            ...
            temp.m_move.m_destinationY = temp.m_wrigglers[i].m_headY;
            temp.m_move.m_depth++;
            temp.m_move.m_parent = explored.size() - 1;
            frontier.push(temp);
  ...
  while(temp.m_move.m_parent != -1)
  {
    temp = explored.at(temp.m_move.m_parent);
    path.push(temp);
  }
