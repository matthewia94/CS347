...
/*******************************************************************************
* @fn solveDLTS
* @brief solves the puzzle using DLTS, will stop at limit and return false
* @pre a valid puzzle to solve
* @post if a solution was found the puzzle will return true
* @limit the limit at which the depth will stop and the function will return 
         false
* @parents a vector of all explored nodes which is where m_parent references to
           to find the parent of a certain state
* @return returns whether or not a solution was found
*******************************************************************************/

/*******************************************************************************
* @fn solveIDDLTS
* @brief solves the puzzle using IDDLTS
* @pre a valid puzzle to solve
* @post if a solution exists it will find the solution and set the calling 
        object's board to the final solution
*******************************************************************************/
...
class Puzzle
{
  private:
    ...
    int findNum(int headX, int headY) const;
    void nextPiece(int& x, int& y) const;
    char convertHead(char dir);
    void findTail(int headX, int headY, int& x, int& y) const;
  ...
  public:
    ...
    bool solveDLTS(int limit, vector<move>& parents);
    void solveIDDLTS();
    int computeHeur() const;
    bool inVisited(vector<Puzzle>& visited);
    void solveGBFGS();
    void solveAStarGS();
